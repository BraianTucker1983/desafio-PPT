class e extends HTMLElement{static get observedAttributes(){return["value"]}constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.render()}attributeChangedCallback(){this.render()}render(){let e=this.getAttribute("value")??"title-font";this.shadow.innerHTML=`
  ...
  `}}customElements.define("custom-text",e);let t=[{pathRegex:/^\/$/,tag:"welcome-page"},{pathRegex:/^\/instructions\/?$/,tag:"instructions-page"},{pathRegex:/^\/move\/?$/,tag:"move-page"},{pathRegex:/^\/result\/?$/,tag:"result-page"}];function s(){let e,s,o=(e="/desafio-PPT",(s=window.location.pathname).startsWith(e)?s.slice(e.length)||"/":s||"/"),i=t.find(e=>e.pathRegex.test(o));if(!i)return void console.warn("Ruta no encontrada:",o);let r=document.querySelector(".root");r&&(r.innerHTML="",r.appendChild(document.createElement(i.tag)))}function o(e){history.pushState({},"","/desafio-PPT"+e),s()}function i(){s(),window.addEventListener("popstate",()=>{s()})}i();class r extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.render()}render(){this.shadow.innerHTML=`
      <style>
        button {
          display: flex;
          align-items: center;
    ...
    `,this.shadow.querySelector("button")?.addEventListener("click",()=>{o("/instructions")})}}customElements.define("custom-button",r);var a={};a=import.meta.resolve("BohIV");var n={};n=import.meta.resolve("2FUzK");var l={};l=import.meta.resolve("4n4pf");class c extends HTMLElement{constructor(){super(),this.selectedOption=null,this.shadow=this.attachShadow({mode:"open"}),this.render(),this.addListeners()}render(){let e=new URL(a).href,t=new URL(n).href,s=new URL(l).href;this.shadow.innerHTML=`
      <style>
        :host {
          display: block;
          position: absolute;
    ...
    `}}customElements.get("custom-score")||customElements.define("custom-score",f);class g extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.render()}render(){this.getAttribute("value"),this.shadow.innerHTML=`
      <style>
        .container {          
          display: flex;
          height: 100vh;          
    ...
    `,this.shadow.querySelector(".start-button")?.addEventListener("click",()=>{o("/instructions")})}}customElements.define("welcome-page",g);class y extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"open"})}connectedCallback(){this.render()}render(){this.shadow.innerHTML=`
      <style>
        .container {                    
          display: flex;
          flex-direction: column;
    ...
    `,this.shadow.querySelector(".move-button")?.addEventListener("click",()=>o("/move"))}}customElements.define("instructions-page",y);class w extends HTMLElement{constructor(){super(),this.selectedMove=null,this.shadow=this.attachShadow({mode:"open"})}connectedCallback(){this.render();let e=this.shadow.querySelector("player-options"),t=this.shadow.querySelector("counter-circle"),s=null;this.shadow.addEventListener("select",e=>{s=e.detail}),t?.addEventListener("finish",()=>{let i=s??p.randomPlay(),r=p.randomPlay();t.hide(),e.showOnly(i),this.shadow.querySelector("computer-option").show(r),p.setMove(i,r),setTimeout(()=>{o("/result")},1e3)})}render(){this.shadow.innerHTML=`
      <style>
        .container {
          display: flex;
          flex-direction: column;
    ...
    `}}customElements.define("move-page",w);let v=new URL(a).href,x=new URL(n).href,b=new URL(l).href,L=new URL(import.meta.resolve("2lEOO")).href,E=new URL(import.meta.resolve("cJ8Te")).href,S=new URL(import.meta.resolve("kMv0Y")).href;class k extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"open"})}connectedCallback(){this.render()}render(){let e=p.getState().currentGame;if(!e){this.shadow.innerHTML="<p>No hay resultado</p>";return}let t=p.winner(e.myPlay,e.computerPlay),s={piedra:v,papel:x,tijera:b},i=s[e.myPlay],r=s[e.computerPlay];this.shadow.innerHTML=`
  <style>
    :host {
      display: block;
      position: relative;
  ...
  <div class="background">
    <img class="computer" src="${r}" alt="Computer Move">
...
//# sourceMappingURL=desafio-PPT.a25b15e1.js.map
